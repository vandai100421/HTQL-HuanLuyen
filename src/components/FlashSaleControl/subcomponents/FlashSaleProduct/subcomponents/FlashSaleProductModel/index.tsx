...
import { FlashSaleProductModel as FlashSaleProductModelType } from "constants/types/flashsale.type";
import {
  forwardRef,
  useEffect,
  useImperativeHandle,
...
type Props = {
  flashsaleProductModelState: State<FlashSaleProductModelType>;
};

type FlashSaleProductModelError = {
  promotion_stock: Array<string>;
};

const validateFlashSaleProductModelSchema = z
  .object({
...
const FlashSaleProductModel = forwardRef(
  ({ flashsaleProductModelState }: Props, ref) => {
    const state = useHookstate(flashsaleProductModelState);
    const [discountPercent, setDiscountPercent] = useState<number>(0);
    const errorState = useHookstate<FlashSaleProductModelError>({
      promotion_stock: [],
    });

    const validateModel = () => {
      const result = validateFlashSaleProductModelSchema.safeParse(
        flashsaleProductModelState.value
      );
...
export default FlashSaleProductModel;
